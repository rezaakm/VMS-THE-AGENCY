...
export class ProjectsService {
  ...
  async getYears() {
    const years = await this.prisma.project.groupBy({ by: ['year'], _count: true, orderBy: { year: 'desc' } });
  ...
  async findOne(id: string) {
    const project = await this.prisma.project.findUnique({
      ...
      include: {
        client: true,
        lineItems: { include: { category: true, vendor: true, alerts: true }, orderBy: { itemNumber: 'asc' } },
      },
    });
    return project;
  }
