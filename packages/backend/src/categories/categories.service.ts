...
export class CategoriesService {
  ...
  async findAll() {
    const categories = await this.prisma.category.findMany({
      include: { _count: { select: { items: true } } }, orderBy: { name: 'asc' },
    });
    ...
    return Promise.all(categories.map(async (cat) => {
      const stats = await this.prisma.lineItem.aggregate({
        ...
        _avg: { unitCost: true, marginPct: true }, _min: { unitCost: true }, _max: { unitCost: true },
      });
      return {
        ...cat, itemCount: cat._count.items,
  ...
  async findOne(id: string) {
    ...
    const items = await this.prisma.lineItem.findMany({
      where: { categoryId: id }, include: { vendor: true, project: { include: { client: true } } },
      orderBy: { createdAt: 'desc' }, take: 50,
    });

    const vendorBreakdown = await this.prisma.lineItem.groupBy({
      by: ['vendorId'], where: { categoryId: id, vendorId: { not: null }, unitCost: { gt: 0 } },
      _count: true, _avg: { unitCost: true },
    });
