...
export class VendorsService {
  ...
  async findAll(options?: { page?: number; limit?: number; search?: string; isActive?: boolean }) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.vendor.findMany({
        where, include: { _count: { select: { lineItems: true } } },
  ...
  async findOne(id: string) {
    const vendor = await this.prisma.vendor.findUnique({
      where: { id }, include: { _count: { select: { lineItems: true } } },
    });
    if (!vendor) return null;

    const categoryBreakdown = await this.prisma.lineItem.groupBy({
      by: ['categoryId'], where: { vendorId: id, categoryId: { not: null } }, _count: true, _avg: { unitCost: true },
    });
    ...
    const recentItems = await this.prisma.lineItem.findMany({
      where: { vendorId: id }, include: { project: { include: { client: true } }, category: true },
      orderBy: { createdAt: 'desc' }, take: 20,
    });

    const financials = await this.prisma.lineItem.aggregate({
      where: { vendorId: id },
      _sum: { totalCost: true, totalSelling: true }, _avg: { unitCost: true, marginPct: true }, _count: true,
    });
