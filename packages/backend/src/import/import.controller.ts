...
export class ImportController {
  ...
  async importFile(@UploadedFile() file: Express.Multer.File, @Body('year') year: string) {
    if (!file) return { success: false, error: 'No file provided' };
    await this.importService.initialize();
    return this.importService.importFromBuffer(file.buffer, file.originalname, parseInt(year) || new Date().getFullYear());
  }

  @Post('bulk')
  @ApiOperation({ summary: 'Bulk import cost sheets from a directory' })
  async bulkImport(@Body() body: { path: string; year: number }) {
    await this.importService.initialize();
