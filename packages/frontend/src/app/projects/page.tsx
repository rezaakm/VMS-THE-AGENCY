...
export default function ProjectsPage() {
  ...
  useEffect(() => { api.projectYears().then(setYears).catch(console.error); }, []);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <select value={selectedYear} onChange={(e) => { setSelectedYear(e.target.value); setPage(1); }} className="input w-40">
          <option value="">All Years</option>
          {years.map((y: any) => <option key={y.year} value={y.year}>{y.year} ({y.count})</option>)}
        </select>
      </div>

      {loading ? <p className="text-gray-400">Loading...</p> : !projects?.items?.length ? <p className="text-gray-400">No projects found.</p> : (
        <>
          <div className="card overflow-hidden">
            <table className="w-full">
              <thead><tr>
                <th className="table-header">Job #</th><th className="table-header">Client</th>
                <th className="table-header">Subject</th><th className="table-header">Year</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {projects.items.map((p: any) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className="table-cell font-mono"><Link href={`/projects/${p.id}`} className="text-brand-500 hover:underline">{p.jobNumber}</Link></td>
                    <td className="table-cell">{p.client.name}</td>
                    <td className="table-cell max-w-xs truncate">{p.subject}</td>
                    <td className="table-cell">{p.year}</td>
                    <td className="table-cell text-right font-medium">{formatOMR(p.totalSell)}</td>
                    <td className="table-cell text-right"><span className={getMarginBadge(p.marginPct)}>{formatPct(p.marginPct)}</span></td>
